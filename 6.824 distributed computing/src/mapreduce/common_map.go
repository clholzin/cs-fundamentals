...
func doMap(
	...
	// both the map task number and the reduce task number.
	...
	for r := 0; r < nReduce; r++ {
		...
		for k := 0; k < len(kv); k++ {
			fmt.Fprintln(reduceFile, fmt.Sprintf(`{"Value":"%s","Key":"%d"}`, kv[k].Key, key))
		}
		reduceFile.Close()
	}
